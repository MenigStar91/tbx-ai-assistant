...
class MockProvider:
    async def generate(self, messages: list[Message]) -> ProviderResponse:
        ...
        if "TABLES" in system or "QUERY_PLANNER" in system:
            ...
            dataset = "account" if "balance" in question and "account" in catalog else "transaction"
            if dataset not in catalog:
                dataset = next(iter(catalog))
            column_names = {item["name"] for item in catalog[dataset]}
            measure_column = next((name for name in ("transaction_amount", "available_balance", "amount", "amount_paise", "value") if name in column_names), None)
